...
STDAPI CreateMemoryAllocator(IMemAllocator **ppAllocator)
{
	...
	HRESULT hr = S_OK;
	*ppAllocator = new CMemAllocator(TEXT("CBaseAllocator"), NULL, &hr);
	(*ppAllocator)->AddRef();
	return hr;
}
...
CBaseOutputPin::BreakConnect()
{
    ...
    if (m_pAllocator) {
        ...
        LONG lRelease = m_pAllocator->Release();
		//<<ke.xu
		//m_pAllocator->Release();
		//ke.xu>>
		AtlTrace(_T("CBaseOutputPin::BreakConnect(), m_pAllocator=0x%x, Release return %d\r\n"), m_pAllocator, lRelease);
        m_pAllocator = NULL;
    }
...
CBaseInputPin::~CBaseInputPin()
{
    if (m_pAllocator != NULL) {
		...
		LONG l = m_pAllocator->Release();
		AtlTrace(_T("CBaseInputPin::~CBaseInputPin(), m_pAllocator=0x%x, release return %d"), m_pAllocator, l);
...
CBaseInputPin::BreakConnect()
{
    ...
    if (m_pAllocator) {
        ...
        long l = m_pAllocator->Release();
		AtlTrace(_T("CBaseInputPin::BreakConnect(), m_pAllocator= 0x%x return %d\r\n"), m_pAllocator, l);
        m_pAllocator = NULL;
    }
