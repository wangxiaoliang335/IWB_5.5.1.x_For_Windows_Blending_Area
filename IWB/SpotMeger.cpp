...
CSpotMerger::CSpotMerger()
{
    ...
    GetModuleFileName(NULL, szModulePath, MAX_PATH); // 获取当前程序完整路径
    PathRemoveFileSpec(szModulePath);                // 去掉文件名，只保留目录
    PathAppend(szModulePath, _T("config.ini"));      // 拼接 INI 文件名
    // 从 config.ini 的 [Settings] 节中读取键 MyValue，默认值为 42
    m_ratio = GetPrivateProfileInt(_T("Settings"), _T("Ratio"), 5, szModulePath);
}
...
void CSpotMerger::DoMerge(TLightSpot* pLightSpots, int* pLightSpotCount)
{
    ...
    while(i < nSpotCount)
    {
        ...
        if(nSpotCount >=1)
        {
         ...
         AtlTrace(_T("DoMerge m_nSeperateX:%d, m_nMergeAreaLeftBorder = %d, m_nMergeAreaRightBorder=%d, eS1Location:%d, s1 ptPosInScreen x:%d\n"),
             m_nSeperateX, m_nMergeAreaLeftBorder, m_nMergeAreaRightBorder, eS1Location, s1.ptPosInScreen.x);
        ...
        if( E_IN_LEFT_MERGE_AREA == eS1Location || E_IN_RIGHT_MERGE_AREA == eS1Location)
        {
            ...
            for(int j = i+1; j < nSpotCount; j++)
            {
                ...
                ESpotLocation eS2Location = E_NOT_IN_MERGE_AREA;//S2所在区域标志1
                ...
                AtlTrace(_T("DoMerge m_nSeperateX:%d, m_nMergeAreaLeftBorder = %d, m_nMergeAreaRightBorder=%d, eS2Location:%d, s2 ptPosInScreen x:%d\n"), 
                        m_nSeperateX, m_nMergeAreaLeftBorder, m_nMergeAreaRightBorder, eS2Location, s2.ptPosInScreen.x);
                ...
                if(eS2Location != E_NOT_IN_MERGE_AREA) 
                {
                    ...
                    AtlTrace(_T("DoMerge MERGE_THRESHOLD:%d, R2 = %d\n"),
                        MERGE_THRESHOLD, R2);
    ...
    if(nSpotCount)
    {
        ...
        for (int index = 0; index < nSpotCount; index++)
        {
            AtlTrace(_T("DoMerge pLightSpots x:%d, pLightSpots y:%d\n"),
                pLightSpots[index].ptPosInScreen.x, pLightSpots[index].ptPosInScreen.y);
        }
    }
#endif
...
//
//    //融合的距离门限的平方
//    int MERGE_THRESHOLD = m_nMergeDistThreshold * m_nMergeDistThreshold * 35;
//
//
//    //融合发生标志
...
//                {
//                    if (cmp_spot.aux.uMergeAreaIndex < 0)
//                    {
//                        cmp_spot.aux.uMergeAreaIndex = spot.aux.uMergeAreaIndex;
//                    }
//                    else if (spot.aux.uMergeAreaIndex < 0)
//                    {
//                        spot.aux.uMergeAreaIndex = cmp_spot.aux.uMergeAreaIndex;
//                    }
//                }
//
//                AtlTrace(_T(" xxxxxx cmp_spot uMergeAreaIndex:%d, spot uMergeAreaIndex:%d\n"), cmp_spot.aux.uMergeAreaIndex, spot.aux.uMergeAreaIndex);
//                if (cmp_spot.aux.uMergeAreaIndex == spot.aux.uMergeAreaIndex)
...
//                    int R2 = dx*dx + dy*dy;//两个光斑之间的距离
//
//                    AtlTrace(_T(" xxxxxx R2:%d, MERGE_THRESHOLD:%d\n"), R2, MERGE_THRESHOLD);
//                    if (R2 < MERGE_THRESHOLD)
//                    {
//                        //光斑距离够近, 需要被合并
...
void CSpotMerger::OnDisplayChange(int nScreenWidth, int nScreenHeight)
{
    ...
    //int nMergeAreaWidth = nScreenWidth >> 6;//除以64  wxlmodify
    int nMergeAreaWidth = nScreenWidth >> m_ratio;//除以16  wxlmodify
    //this->m_nMergeAreaLeftBorder  = m_nSeperateX - nMergeAreaWidth/2;  //wxl modify
    //this->m_nMergeAreaRightBorder = m_nSeperateX + nMergeAreaWidth/2; //wxl modify

    this->m_nMergeAreaLeftBorder  = m_nSeperateX - nMergeAreaWidth/1;
    this->m_nMergeAreaRightBorder = m_nSeperateX + nMergeAreaWidth/1;
    ...
    m_nMergeDistThreshold = nMergeAreaWidth*2/1;
}
