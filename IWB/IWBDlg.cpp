...
//HRESULT CIWBDlg::OnAbortManualCalibrating (WPARAM wParam,LPARAM lParam)
//{
//
//    //g_oMouseEventGen.LeaveCalibrateMode();
//
//    if(this->m_tOldState.bEnableOpticalPen)//恢复原来的控制状态
//    {
//        //g_oMouseEventGen.EnableOpticalPenControl(TRUE);
//    }
//
////    m_bNeedManualCalibrate = FALSE;
//
//    return 0;
//}
...
//@功能:从配置文件中读取校正参数
//BOOL CIWBDlg::LoadCalibParams(TCalibParams& calibParams, const TCHAR* szProfileName)
//{
//    BOOL bRet = TRUE;
//
//    calibParams.szImage.cx  = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateImageWidth"  ), 640,   szProfileName);
//    calibParams.szImage.cy  = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateImageHeight" ), 480,   szProfileName);
//    calibParams.szScreen.cx = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateScreenWidth" ), GetSystemMetrics(SM_CXVIRTUALSCREEN),  szProfileName);
//    calibParams.szScreen.cy = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateScreenHeight"), GetSystemMetrics(SM_CYVIRTUALSCREEN),   szProfileName);
//
//    UINT uMonitorCount = GetPrivateProfileInt(_T("Calibrate"), _T("MonitorCount"), 0,   szProfileName);
//    calibParams.allCalibCoefs.resize(uMonitorCount);
//
//    for(UINT monitorId = 0; monitorId < calibParams.allCalibCoefs.size(); monitorId++)
//    {//对每个屏幕循环
//        CString strSectionName;
//        CString strKeyName;
//        strSectionName.Format(_T("Monitor%d"), monitorId);
//
//        UINT uParamCount = GetPrivateProfileInt(strSectionName, _T("ParamCount"), 0,   szProfileName);
//
//        calibParams.allCalibCoefs[monitorId].resize(uParamCount);
//
//        for(UINT paramId = 0; paramId < uParamCount; paramId ++)
//        {//对每个参数循环
//
//            strKeyName.Format(_T("Param%02d"), paramId);
//
//            TCHAR szText[128];
//            memset(szText, 0, sizeof(szText));
//            GetPrivateProfileString(strSectionName, strKeyName, _T(""), szText, _countof(szText), szProfileName);
//
//            calibParams.allCalibCoefs[monitorId][paramId] = _tstof(szText);                
//
//
//        }//for-each param
//
...
//BOOL CIWBDlg::LoadCfgData()
//{
//
//    BOOL bRet = FALSE;
//
//    DWORD dwThreshold = GetPrivateProfileInt(_T("Settings"), _T("Y_Threshold"), DEFAULT_DETECT_THRESHOLD,  PROFILE::PROFILE_NAME);
//    if(dwThreshold > 255) dwThreshold = 255;
//    m_SysCfgData.cYThreshold = (BYTE)dwThreshold;
//    m_SysCfgData.tAdvancedSettings.nBrightnessThreshold = (BYTE)dwThreshold;     //zwn 2013.06.21
//
//    //BOOL bEnableOpticalPenControl = GetPrivateProfileInt(_T("Settings"), _T("EnableOpticalControl"), FALSE, m_strProfilePath);
//    //m_bEnableOpticalPenControl = bEnableOpticalPenControl;
//
//    TCHAR szText[1024];
//    memset(szText, 0, sizeof(szText));
//    GetPrivateProfileString(_T("Device"), _T("FavoriteDevicePath"), _T(""), szText, _countof(szText),  PROFILE::PROFILE_NAME);
//    m_SysCfgData.strFavoriteDevicePath = szText;
//
//    //TCHAR szBuf[1024];
//    //memset(szBuf, 0, sizeof(szBuf));
//    //GetPrivateProfileString(_T("Device"), _T("CameraId"), _T(""), szBuf, _countof(szBuf), m_strProfilePath);
//    //CString strUSBCameraId = szBuf;
//
//
//    //memset(szBuf, 0, sizeof(szBuf));
//    //GetPrivateProfileString(_T("Device"), _T("CameraFriendlyName"), _T(""), szBuf, _countof(szBuf), m_strProfilePath);
//    //CString strUSBCameraFrendilyName = szBuf;
//
//    //const TCaptureDeviceInstance* pDevInst = NULL;
//
//    //if(strUSBCameraId.GetLength() != 0 && strUSBCameraFrendilyName.GetLength() !=0)
//    //{
//    // pDevInst = GetCaptureDevByIdAndFrendilyName(strUSBCameraId, strUSBCameraFrendilyName);
//
//    //}
//    //else if(strUSBCameraId.GetLength() != 0 )
//    //{
//    // pDevInst = GetCaptureDevById(strUSBCameraFrendilyName);
//    //}
//    //else
//    //{
//    // pDevInst = GetDefaultUSBDev();
//    //}
//
//    ////const TCaptureDeviceInstance* pDevInst = GetCaptureDevByName(szBuf);
//
//    //if(pDevInst)
//    //{
//    // m_SysCfgData.tCapDevInstSelected = *pDevInst;
//    //}
//    //else
//    //{
//    // m_SysCfgData.tCapDevInstSelected.m_nInstNo = 1;
//    // m_SysCfgData.tCapDevInstSelected.m_strName = DEFAULT_FAVORITE_CAP_DEV_NAME;
//    // m_SysCfgData.tCapDevInstSelected.m_strDevPath = DEFAULT_DEV_IDS[0];
//    // m_SysCfgData.tCapDevInstSelected.m_vecVideoFmt.clear();
//    // m_SysCfgData.tCapDevInstSelected.m_vecVideoFmt.push_back(DEFAULT_VIDEO_FORMAT);
//    //}
//
//
//    GetPrivateProfileString(_T("Device"), _T("FavoriteVideoFormat"), DEFAULT_FAVORITE_VIDEO_FORMAT_NAME, szText, _countof(szText),  PROFILE::PROFILE_NAME);
//    m_SysCfgData.strFavoriteMediaType = szText;
//
//
//    //载入VideoProcAmp属性
//    // PROFILE::LoadCameraParamsForNormalUsage(m_SysCfgData.tCameraParsmsForNormalUsage);
//
//    //const VideoMediaType* pVideoFmt = GetVideoFormat(m_SysCfgData.tCapDevInstSelected, szBuf);
//    //if(pVideoFmt)
//    //{
//    // m_SysCfgData.tMediaTypeSelected = *pVideoFmt;
//    //}
//    //else
//    //{
//    // m_SysCfgData.tMediaTypeSelected = DEFAULT_VIDEO_FORMAT;
//    //}
//
//
//    //高级设定, 2012-06-11, added by toxuke@gmail.com
//    m_SysCfgData.tAdvancedSettings.nAutoMaskDilateRadius = PROFILE::GetClutterMaskAreaInflationRadius();
//    m_SysCfgData.tAdvancedSettings.nFirstGuideLineYPos = PROFILE::GetFirstGuidelineYPos();
//    m_SysCfgData.tAdvancedSettings.bFirstGuideLineVisible = PROFILE::IsFirstGuidelineVisible();
//    m_SysCfgData.tAdvancedSettings.nSecondGuideLineYPos = PROFILE::GetSecondGuidelineYPos();
//    m_SysCfgData.tAdvancedSettings.bSecondGuideLineVisible = PROFILE::IsSecondGuidelineVisible();
//
//    m_SysCfgData.tAdvancedSettings.nSpotProportion = PROFILE::GetSpotProportionPos();
//    m_SysCfgData.tAdvancedSettings.nMultEraser = PROFILE::GetMultiplesEraser();
//    m_SysCfgData.tAdvancedSettings.nFixedBlobSetTime = PROFILE::GetFixedBlobSetTime();
//    m_SysCfgData.tAdvancedSettings.nNormalUserBrightness = PROFILE::GetNormalUseBrightness();
//    m_SysCfgData.tAdvancedSettings.nAutoMaskDetectThreshold = PROFILE::GetAutoMaskingTheshold();
//
//    //<<added by toxuke@gmail.com, 2012/08/20
//    m_SysCfgData.tAdvancedSettings.nXDeviationCompensationValue = PROFILE::GetXDeviationCompensationVal();
//    m_SysCfgData.tAdvancedSettings.nYDeviationCompensationValue = PROFILE::GetYDeviationCorrectVal();
//    m_SysCfgData.tAdvancedSettings.nAutoCalibrateavebrightness  = PROFILE::GetAutoCalibrateImageAvgBrightness();
//    m_SysCfgData.tAdvancedSettings.nAutoCalibratehilightgray    = PROFILE::GetAutoCalibrateHilightGray();
//
//
//    //added by toxuke@gmail.com, 2012/08/20>>
//
...
//
//    if (theApp.GetUSBKeyType())
//    {
//        m_SysCfgData.tAdvancedSettings.bGuestureRecognition =PROFILE::IsGestureRecognitionAvailable();
//    }
//    else
//    {
//        m_SysCfgData.tAdvancedSettings.bGuestureRecognition = theApp.GetUSBKeyType();
//    }
//
//    m_SysCfgData.tAdvancedSettings.bRecordVideo = PROFILE::IsRecordVideo();
//    m_SysCfgData.tAdvancedSettings.bDoubleScreen = theApp.GetDoubleScreenMerge();
//    m_SysCfgData.tAdvancedSettings.bIsRearProjection = PROFILE::IsOpenRearProjection();
//
//    //>>
//
//    /*
//    TCalibrateData calibrateData;//校正数据
//
//    //校正数据载入
//    calibrateData.nCalibrateImageWidth = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateImageWidth" ), 640,  PROFILE::PROFILE_NAME);
//    calibrateData.nCalibrateImageHeight = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateImageHeight"), 480,  PROFILE::PROFILE_NAME);
//
//    calibrateData.nCalibrateScreenWidth = GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateScreenWidth"), 1024,  PROFILE::PROFILE_NAME);
//    calibrateData.nCalibrateScreenHeight= GetPrivateProfileInt(_T("Calibrate"), _T("CalibrateScreenHeight"), 768,  PROFILE::PROFILE_NAME);
//
//    UINT calibratePointsNumber = GetPrivateProfileInt(_T("Calibrate"), _T("CalibratePtNumber"), -1,  PROFILE::PROFILE_NAME);
//    */
//
//    TCalibParams calibParams;
//
//    LoadCalibParams(calibParams, PROFILE::PROFILE_NAME);
//
//
//
//    if(calibParams.allCalibCoefs.size())
//        //if(calibratePointsNumber != (UINT)(-1))
//    {
//        /*
//        calibrateData.vecCalibratePoints.resize(calibratePointsNumber);
//        GetPrivateProfileStruct(_T("Calibrate"), _T("CalibrateData"), &calibrateData.vecCalibratePoints[0], calibratePointsNumber* sizeof(CalibratePair),  PROFILE::PROFILE_NAME);
//        g_oVideo2ScreenMap.SetCalibrateData(calibrateData);
//
//        TCHAR szIsValidManualData[4];
//        memset(szIsValidManualData,0,sizeof(szIsValidManualData));
//        GetPrivateProfileString(_T("Calibrate"), _T("IsValidManualCorrectData"),_T("No"),szIsValidManualData,_countof(szIsValidManualData),  PROFILE::PROFILE_NAME);
//
//        if(_tcsicmp(szIsValidManualData, _T("Yes")) == 0)
//        {
//
//        //<<2012/07/20, 载入手动补正数据
//        std::vector<CalibratePair> vecManualCorrectData;
//        vecManualCorrectData.resize(4);
//
//        BOOL bSuccess = GetPrivateProfileStruct(_T("Calibrate"), _T("ManualCorrectData"), &vecManualCorrectData[0], 4*sizeof(CalibratePair),  PROFILE::PROFILE_NAME);
//        if(bSuccess)
//        {
//        g_oVideo2ScreenMap.UpdateManualCorrectData(vecManualCorrectData);
//        }
//        }
//        //2012/07/12>>
//
//
//
//        if(g_oVideo2ScreenMap.DoCalibrate())
//        {
//        g_oMouseEventGen.EnableOpticalPenControl(TRUE);//使能光笔控制
//
//        //设置校正补偿值
//        g_oVideo2ScreenMap.SetDeviationCompensationValue(
//        this->m_SysCfgData.tAdvancedSettings.nXDeviationCompensationValue,
//        this->m_SysCfgData.tAdvancedSettings.nYDeviationCompensationValue);
//
//        }
//        */
//
//        g_oVideo2ScreenMap.SetCalibParams(calibParams);
//        g_oMouseEventGen.EnableOpticalPenControl(TRUE);//使能光笔控制
//
//
//
//
//    }
//    else//未发现校正数据, gamma值回到40, added by toxuke@gmail.com, 2012-06-07
//    {
//        m_SysCfgData.tCameraParsmsForNormalUsage.Prop_VideoProcAmp_Gamma = 40;
//        g_oMouseEventGen.EnableOpticalPenControl(FALSE);//禁用光笔控制
//    }
//
//    if(PROFILE::GetDisableOpticalPenMouseControl())//配置文件中强制禁止了光笔控制
//    {
//        g_oMouseEventGen.EnableOpticalPenControl(FALSE);//<<Notice!!!
//    }
//
//
//    return bRet;
...
////@功能:保存校正参数到配置文件中去
//BOOL CIWBDlg::SaveCalibParams(const TCalibParams& calibParams, const TCHAR* szProfileName)
//{   BOOL bRet = TRUE;
//
//    CString strKeyValue;
//
//    //保存校正时的图像尺寸
//    strKeyValue.Format(_T("%d"), calibParams.szImage.cx);
//    WritePrivateProfileString(_T("Calibrate"), _T("CalibrateImageWidth"), strKeyValue, szProfileName);
//
//    strKeyValue.Format(_T("%d"), calibParams.szImage.cy);
//    WritePrivateProfileString(_T("Calibrate"), _T("CalibrateImageHeight"), strKeyValue,  szProfileName);
//
//
//    //保存校正时屏幕尺寸
//    strKeyValue.Format(_T("%d"), calibParams.szScreen.cx);
//    WritePrivateProfileString(_T("Calibrate"), _T("CalibrateScreenWidth"), strKeyValue,  szProfileName);
//
//    strKeyValue.Format(_T("%d"), calibParams.szScreen.cy);
//    WritePrivateProfileString(_T("Calibrate"), _T("CalibrateScreenHeight"), strKeyValue,  szProfileName);
//
//
//    //保存校正时的屏幕个数
//    strKeyValue.Format(_T("%d"), calibParams.allCalibCoefs.size());
//    WritePrivateProfileString(_T("Calibrate"), _T("MonitorCount"), strKeyValue,  szProfileName);
//
//    //
//    for(UINT monitorId = 0; monitorId < calibParams.allCalibCoefs.size(); monitorId++)
//    {
...
//
//        strSectionName.Format(_T("Monitor%d"), monitorId);
//        strKeyValue.Format(_T("%d"), calibParams.allCalibCoefs[monitorId].size());
//        WritePrivateProfileString(strSectionName, _T("ParamCount"), strKeyValue,  szProfileName);
//
//
//        for(UINT paramId = 0; paramId < calibParams.allCalibCoefs[monitorId].size(); paramId ++)
//        {
//
//            strKeyName.Format(_T("Param%02d"), paramId);
//
//            strKeyValue.Format(_T("%.31E"), calibParams.allCalibCoefs[monitorId][paramId]);
//            WritePrivateProfileString(strSectionName, strKeyName, strKeyValue,  szProfileName);
//
//        }
//
//    }//for-each(i)
...
//BOOL CIWBDlg::SaveCfgData()
//{
//    CString strValue;
//
//    strValue.Format(_T("%d"), m_SysCfgData.cYThreshold);
//    WritePrivateProfileString(_T("Settings"), _T("Y_Threshold"), strValue,  PROFILE::PROFILE_NAME);
//
//    WritePrivateProfileString(_T("Device"), _T("FavoriteDevicePath"), m_SysCfgData.strFavoriteDevicePath,  PROFILE::PROFILE_NAME);
//
//    //strValue = GetVideoFormatName(m_SysCfgData.tMediaTypeSelected);
//    WritePrivateProfileString(_T("Device"), _T("FavoriteVideoFormat"), m_SysCfgData.strFavoriteMediaType,  PROFILE::PROFILE_NAME);
//
//
//    //保存校正数据
//    const TCalibParams* pCalibParams = g_oVideo2ScreenMap.GetCalibParams();
//    if(pCalibParams)
//    {
//        SaveCalibParams(*pCalibParams, PROFILE::PROFILE_NAME);
//    }
//    //   strValue.Format(_T("%d"), calibParams.szImage.cx);
//    //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateImageWidth"), strValue,  PROFILE::PROFILE_NAME);
//
//    //strValue.Format(_T("%d"), calibParams.szImage.cy);
//    //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateImageHeight"), strValue,  PROFILE::PROFILE_NAME);
//
//
//    //   strValue.Format(_T("%d"), calibParams.szScreen.cx);
//    //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateScreenWidth"), strValue,  PROFILE::PROFILE_NAME);
//
//    //strValue.Format(_T("%d"), calibParams.szScreen.cy);
//    //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateScreenHeight"), strValue,  PROFILE::PROFILE_NAME);
//
//
//    /*
//    strValue.Format(_T("%d"), calibrateData.vecCalibratePoints.size());
//    WritePrivateProfileString(_T("Calibrate"), _T("CalibratePtNumber"), strValue,  PROFILE::PROFILE_NAME);
//
//    WritePrivateProfileStruct(_T("Calibrate"), _T("CalibrateData"), const_cast<LPVOID>((LPVOID)&calibrateData.vecCalibratePoints[0]), calibrateData.vecCalibratePoints.size()* sizeof(CalibratePair),  PROFILE::PROFILE_NAME);
//
//    */
//    if(PROFILE::PROFILE_NAME != PROFILE::PROFILE_SYSTEM_NAME)
//    {
//
//        if(pCalibParams)
//        {
//            SaveCalibParams(*pCalibParams, PROFILE::PROFILE_SYSTEM_NAME);
//        }
//
//        //strValue.Format(_T("%d"), m_SysCfgData.cYThreshold);
//        //WritePrivateProfileString(_T("Settings"), _T("Y_Threshold"), strValue,  PROFILE::PROFILE_SYSTEM_NAME);
//        //WritePrivateProfileString(_T("Device"), _T("FavoriteDevicePath"), m_SysCfgData.strFavoriteDevicePath,  PROFILE::PROFILE_SYSTEM_NAME);
//        //WritePrivateProfileString(_T("Device"), _T("FavoriteVideoFormat"), m_SysCfgData.strFavoriteMediaType,  PROFILE::PROFILE_SYSTEM_NAME);
//
//        //strValue.Format(_T("%d"),  calibParams.szImage.cx);
//        //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateImageWidth"), strValue,  PROFILE::PROFILE_SYSTEM_NAME);
//
//        //strValue.Format(_T("%d"), calibParams.szImage.cy);
//        //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateImageHeight"), strValue,  PROFILE::PROFILE_SYSTEM_NAME);
//
//
//        //      strValue.Format(_T("%d"), calibParams.szScreen.cx);
//        //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateScreenWidth"), strValue,  PROFILE::PROFILE_SYSTEM_NAME);
//
//        //strValue.Format(_T("%d"), calibParams.szScreen.cy);
//        //WritePrivateProfileString(_T("Calibrate"), _T("CalibrateScreenHeight"), strValue,  PROFILE::PROFILE_SYSTEM_NAME);
//
//
//        //strValue.Format(_T("%d"), calibrateData.vecCalibratePoints.size());
//        //WritePrivateProfileString(_T("Calibrate"), _T("CalibratePtNumber"), strValue,  PROFILE::PROFILE_SYSTEM_NAME);
//
//        //WritePrivateProfileStruct(_T("Calibrate"), _T("CalibrateData"), const_cast<LPVOID>((LPVOID)&calibrateData.vecCalibratePoints[0]), calibrateData.vecCalibratePoints.size()* sizeof(CalibratePair),  PROFILE::PROFILE_SYSTEM_NAME);
//
//    }
//
//
//    //保存VideoProcAmp属性
//    //PROFILE::SaveVideoProcAmpProperty(m_SysCfgData.tCameraParsmsForNormalUsage);
//
//
//    //高级设定, 2012-06-11, added by toxuke@gmail.com
//    PROFILE::SaveClutterMaskAreaInflationRadius(m_SysCfgData.tAdvancedSettings.nAutoMaskDilateRadius);
//    PROFILE::SaveFirstGuidelineYPos(m_SysCfgData.tAdvancedSettings.nFirstGuideLineYPos);
//    PROFILE::SaveFirstGuidelineVisibility(m_SysCfgData.tAdvancedSettings.bFirstGuideLineVisible);
//    PROFILE::SaveSecondGuidelineYPos(m_SysCfgData.tAdvancedSettings.nSecondGuideLineYPos );
//    PROFILE::SaveSecondGuidelineVisibility(m_SysCfgData.tAdvancedSettings.bSecondGuideLineVisible);
//    PROFILE::SaveSpotProportionPos(m_SysCfgData.tAdvancedSettings.nSpotProportion);
//    PROFILE::SaveMultiplesEraser(m_SysCfgData.tAdvancedSettings.nMultEraser);
//    PROFILE::SaveFixedBlobSetTime(m_SysCfgData.tAdvancedSettings.nFixedBlobSetTime);
//    PROFILE::SaveAutoMaskingTheshold(m_SysCfgData.tAdvancedSettings.nAutoMaskDetectThreshold);
//
//    //<<added by toxuke@gmail.com, 2012/08/20
//    PROFILE::SaveXDeviationCompensationVal(m_SysCfgData.tAdvancedSettings.nXDeviationCompensationValue);
//    PROFILE::SaveYDeviationCorrectVal(m_SysCfgData.tAdvancedSettings.nYDeviationCompensationValue);
//    //added by toxuke@gmail.com, 2012/08/20>>
//
//    //<<added by toxuke@gmail.com, 2013/04/19
//    PROFILE::SaveGestureRecognitionSetting(m_SysCfgData.tAdvancedSettings.bGuestureRecognition);
//    PROFILE::SaveNormalUseBrightness(m_SysCfgData.tAdvancedSettings.nNormalUserBrightness);
//    PROFILE::SaveRecordVideo(m_SysCfgData.tAdvancedSettings.bRecordVideo);
//    PROFILE::SaveAutoCalibrateImageAvgBrightness(m_SysCfgData.tAdvancedSettings.nAutoCalibrateavebrightness);
//    PROFILE::SaveAutoCalibrateHilightGray(m_SysCfgData.tAdvancedSettings.nAutoCalibratehilightgray) ;
//    PROFILE::SaveOpenRearProjection(m_SysCfgData.tAdvancedSettings.bIsRearProjection);
//
//    //>>
//
//    //2012/07/20, 保存手动补正数据
//    /*   if(g_oVideo2ScreenMap.IsValidManualCorrectData())
//    {
//
//    LPVOID lpData = (LPVOID)&g_oVideo2ScreenMap.GetManualCorrectData()[0];
//    size_t nSize = g_oVideo2ScreenMap.GetManualCorrectData().size() * sizeof(CalibratePair);
//    WritePrivateProfileStruct(_T("Calibrate"), _T("ManualCorrectData"), lpData, nSize,  PROFILE::PROFILE_NAME);
//
//    strValue = _T("Yes");
//    WritePrivateProfileString(_T("Calibrate"), _T("IsValidManualCorrectData"), strValue,  PROFILE::PROFILE_NAME);
//    }
//    else
//    {
//    strValue = _T("No");
